...
public class IntermediaWhoWeArePage {
    ...
    public void checkAndrewGName() {
        Assert.assertEquals(ImPagesData.getProperty("expectedAndrewG_name"), nameAndrewG.getText());
    }

    public void checkAndrewGDuty(){
        Assert.assertEquals(ImPagesData.getProperty("expectedAndrewG_duty"), dutyAndrewG.getText());
    }
    ...
    public List<WebElement> getAllAndrewGBio() {
        //buttonAndrewGPhoto.click();
        return allBioOfAndrew.findElements(By.tagName("p"));
        /*for (int i = 0; i < paragraphsOfAndrewBio.size(); i++) {
            //Assert.assertEquals(expectedResultsAndrewGDetailedInfo[i], paragraphsOfAndrewBio.get(i).getText());
        }*/ // написать умную провпрку на то, что одна из проверок завалится, но остальные должны быть выполнены в любом
          // случае.
    }
